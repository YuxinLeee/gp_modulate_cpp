...
lpvDS::lpvDS(int K, int M, const MatrixXd Priors_fMatrix, const MatrixXd Mu_fMatrix, const MatrixXd Sigma_fMatrix, const MatrixXd A_fMatrix, const VectorXd att_fVector):K_(K),M_(M) {
...
void lpvDS::setup_params()
{
    ...
    att_      = new VectorXd; att_->resize(M_);
...
void lpvDS::initialize_att(const VectorXd fMatrix ){
    // cout<<"** Initializing Attractor **"<< endl;
    // if (fMatrix.rows() != M_) {
    //     cout<<"Initialization of Attractor is wrong."<<endl;
    //     cout<<"Dimension of states is: "<<M_<<endl;
    //     cout<<"Dimension of Attractor is: "<<fMatrix.rows()<<endl;
    //     ERROR();
    // }
    
    *att_ = fMatrix;
    std::cout << "Attractor: " << *att_ << std::endl;
}
...
VectorXd   lpvDS::compute_f(VectorXd xi){
